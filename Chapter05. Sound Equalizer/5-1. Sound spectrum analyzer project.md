...
class PixelLedMatrixPIO:
    def __init__(self, pin, width=WIDTH, height=HEIGHT, sm_id=2, freq=8_000_000):
...
matrix = PixelLedMatrixPIO(pin=0)

band_freqs = logspace(FREQ_MIN, FREQ_MAX, WIDTH)

i2s_buf = bytearray(READ_BYTES)
raw_hold = []

def _pull_raw_i32(target_count):
    while len(raw_hold) < target_count:
        read_len = audio_in.readinto(i2s_buf)
        if read_len is None or read_len == 0:
            utime.sleep_ms(1)
            continue
        n_frames = read_len // FRAME_BYTES
        for i in range(n_frames):
            raw_hold.append(struct.unpack_from('<i', i2s_buf, 4 * i)[0])

def read_frame_goertzel(n):
    decim = SAMPLE_RATE // SAMPLE_RATE_PROC
    need = n * decim
    _pull_raw_i32(need)
    raw = raw_hold[:need]
    del raw_hold[:need]
    decimated = raw[::decim]
    full_scale = (1 << 31) - 1
    frame = [s / full_scale for s in decimated]
    m = sum(frame) / n
    for i in range(n):
        frame[i] -= m
    return frame, raw
...
def main():
    ...
    while True:
        frame, raw = read_frame_goertzel(FRAME_SAMPLES)
        powers = [goertzel_power(frame, f, SAMPLE_RATE_PROC, FRAME_SAMPLES) for f in band_freqs]
        ...
        now = utime.ticks_ms()
        if utime.ticks_diff(now, last_print) >= 50:
            db = calc_rms_db(raw)
            last_print = now

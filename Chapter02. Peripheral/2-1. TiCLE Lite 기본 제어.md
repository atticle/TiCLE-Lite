...
실습을 시작하기 전 TiCLE Lite의 Power 보드와 Basic 보드간의 전원 연결이 필요합니다. 아래 사진과 같이 Power 보드의 5V/GND를 Basic 보드의 5V/GND에 연결합니다. 반드시 다른 전압이 아닌 **5V 전압을 연결하도록 합니다.**

![power_connect](pds/power_connect.png)

이후 후술할 모든 핀 연결은 뒷장에 프로젝트 개발에 사용되는 핀과 동일하게 사용됩니다. 따라서 핀 연결 및 실습 완료 후 선 연결을 제거하지 않으시기를 권장합니다.
...
![switch_pic](pds/switch_pic.png)

Switch는 Basic 보드에 위치하여 있기에 별도의 핀 연결이 필요합니다. 아래의 사진 및 표를 참고하여 Core 보드의 핀과 Basic 보드의 핀을 연결해줍니다.

![switch_connect](pds/switch_pin_connect.png)
...
![ir_sensor](pds/ir_pic.png)

아래 사진과 표를 참고하여 Core 보드의 핀과 Basic 보드의 핀을 연결해줍니다.

![ir_pin_connect](pds/ir_pin_connect.png)
...
class IRNECPoll:
    ...
    def _wait_edge(self, to_ms):
        t0 = utime.ticks_ms()
        lv = self.p.value()
        while utime.ticks_diff(utime.ticks_ms(), t0) < to_ms:
            v = self.p.value()
            if v != lv:
                return lv, utime.ticks_us()
        return None, None
    ...
    def _capture(self, start_to_ms=200, gap_us=14000, min_us=100, max_ms=120):
        ...
        seg, start_ms, last = [], utime.ticks_ms(), t0
        while utime.ticks_diff(utime.ticks_ms(), start_ms) < max_ms:
            v = self.p.value()
            if v != lv:
                t = utime.ticks_us()
                d = utime.ticks_diff(t, last)
                if d >= min_us: seg.append((lv, d))
                lv, last = v, t
            else:
                if utime.ticks_diff(utime.ticks_us(), last) > gap_us and len(seg) > 8:
                    break
    ...
    def read(self, start_to_ms=200, gap_us=14000, min_us=100, max_ms=120, repeat_window_ms=400):
        ...
        if r[0]=="REPEAT":
            if self.last and utime.ticks_diff(utime.ticks_ms(), self.last_ts)<=repeat_window_ms:
                return self.last
            return None
        if r[0] in ("NEC","NECX"):
            self.last = (r[1], r[2])
            self.last_ts = utime.ticks_ms()
            return self.last
        return None
...
- https://github.com/hanback-lab/TiCLE-Lite/blob/main/lib/ir_remote.py
...
아래 사진과 같이 위치한 WS2812를 제어해봅니다. 핀 연결은 기본적으로 연결이 되어져 있습니다. 기본 핀 연결은 **3** 번 핀입니다.

![ws2812](pds/ws2812_pic.png)
...
matrix = WS2812MatrixPIO(pin=3, width=16, height=16)
matrix.clear()
matrix[3, 4] = (255, 0, 0)
matrix[4, 4] = (0, 255, 0)
matrix[5, 4] = (0, 0, 255)
...
- https://github.com/hanback-lab/TiCLE-Lite/blob/main/lib/ws2812.py
...
아래는 WS2812 Matrix를 제어하는 클래스 API 문서입니다.

**WS2812Matrix 클래스**
- `WS2812Matrix(pin_sm_pairs, panel_width=16, panel_height=16, grid_width=1, grid_height=1, zigzag=False, origin="top_left", brightness=0.25)` : 여러 개의 RP2350 PIO(StateMachine)과 핀 조합을 이용해 WS2812 LED 매트릭스를 초기화합니다. 그리드 단위로 여러 패널을 배치하거나, 지그재그·기준 방향·밝기를 설정하실 수 있습니다.
- `update(wait=True) -> None` : 프레임버퍼의 변경 내용을 DMA를 통해 WS2812로 전송합니다. wait=False로 지정하면 전송 완료를 기다리지 않고 즉시 반환합니다.
- `clear() -> None` : 모든 픽셀을 꺼짐(검정색)으로 설정하고 즉시 화면을 갱신합니다.
- `fill(color) -> None` : 전체 매트릭스를 지정한 색으로 채웁니다. 실제 표시에는 update() 호출이 필요합니다.
- `deinit() -> None` : 모든 LED를 소등한 뒤 PIO 상태머신을 비활성화하여 종료합니다.
- `draw_line(x0, y0, x1, y1, color) -> None` : 지정한 두 점을 잇는 직선을 그립니다.
- `draw_line_polar(cx, cy, length, angle_deg, color) -> None` : 중심점, 길이, 각도를 이용해 극좌표 형태의 직선을 그립니다.
- `draw_rect(x, y, w, h, outline, fill=None) -> None` : 사각형을 그립니다. fill 인자를 지정하면 내부를 채웁니다.
- `draw_rect_polar(cx, cy, w, h, angle_deg, outline, fill=None) -> None` : 중심과 회전각을 기준으로 회전된 사각형을 그립니다.
- `draw_ellipse(cx, cy, rx, ry=None, outline=(255,255,255), fill=None, angle_deg=0.0) -> None` : 타원을 그립니다. ry가 없으면 원으로 간주되며, angle_deg로 회전시킬 수 있습니다.
- `draw_circle(cx, cy, r, color, fill=None) -> None` : 원을 그립니다. 내부 채우기(fill) 옵션이 있습니다.
- `width -> int` : 전체 매트릭스의 폭(픽셀 단위)입니다.
- `height -> int` : 전체 매트릭스의 높이(픽셀 단위)입니다.
- `brightness -> float` : 현재 밝기(0.0~1.0)이며, 설정 시 내부 LUT가 자동으로 갱신됩니다.
...
![vr_pin_connect](pds/ir_pin_connect.png)
...
![cds_pin_connect](pds/cds_pin_connect.png)
...
![mic_pin_connect](pds/mic_pin_connect.png)
...
![piezo_buzzer](pds/piezo_pic.png)

아래 사진과 표를 참고하여 Core 보드의 핀과 Basic 보드의 핀을 연결해줍니다.

![piezo_pin_connect](pds/piezo_pin_connect.png)
...
Servo motor는 기본적으로 `2` 번 핀에 연결이 되어져 있습니다.
...
이러한 특징은 I2C의 두 핀(scl/sda) 연결만으로도 I2C 라인에 위치한 모든 주변장치들을 제어할 수 있다는 것을 의미합니다. 사진과 표를 참고하여 Core 보드의 핀과 Basic 보드의 핀을 연결해줍니다.

![i2c_pin_connect](pds/i2c_pin_connect.png)
...
I2C 통신은 하나의 버스라인에 여러 장치가 연결되었기에, 각 장치를 인식하기 위한 **주소** 가 필요합니다. PCF8574의 주소는 `0x3F` 이며, 변하지 않는 고정 값이기에 상수로 지정해둡니다.
...
- https://github.com/hanback-lab/TiCLE-Lite/blob/main/lib/hd44780_pcf8574.py

<details>
<summary>hd44780_pcf8574.py</summary>
...
아래는 LCD를 제어하는 클래스 API 문서입니다.

**HD44780_PCF8574 클래스**
* `HD44780_PCF8574(scl, sda, *, addr=0x3F, freq=400_000, cols=16, rows=2)`:  PCF8574 I2C 익스펀더를 통해 HD44780 LCD를 초기화합니다. I²C 주소/속도와 컬럼·행 크기를 지정하실 수 있습니다.
* `deinit() -> None` : 표시/커서를 끄고 화면을 지운 뒤 종료합니다.
* `clear() -> None` : 화면을 초기화하고 커서를 (0,0)으로 이동합니다.
* `home() -> None` :  커서를 (0,0)으로 복귀시키고 셰프트를 초기화합니다.
* `set_display(on: bool|None=None, cursor: bool|None=None, blink: bool|None=None) -> None` : 표시 on/off, 커서 표시, 깜박임을 설정합니다. `None`은 기존 값을 유지합니다.
* `text(text: str|bytes, x: int|None=None, y: int|None=None, *, wrap: bool=False) -> None` : 지정 위치에 문자열을 씁니다. `wrap=True`이면 화면 폭을 넘어갈 때 다음 줄로 자동 개행합니다.
* `scroll_left(n: int=1) -> None` / `scroll_right(n: int=1) -> None` : 내용(DDRAM)을 왼쪽/오른쪽으로 n칸 스크롤합니다.
...
![imu](pds/mpu6050_pic.png)

Basic 보드에 설치된 기준으로 세 축의 위치는 다음과 같습니다.

![imu_axis](pds/imu_axis.png)

...
* Auth Token 등록
* Blynk 앱에서 `V0`, `V1` 등 가상핀 설정

아래 링크에 접속하여 라이브러리 파일을 다운로드 받습니다.

- https://github.com/hanback-lab/TiCLE-Lite/blob/main/lib/blynk_mqtt.py

VSCode 터미널을 열어 아래와 같이 입력합니다.

```
replx put blynk_mqtt.py /lib/
```
...
class HD44780_PCF8574:
    def __init__(self, scl=13, sda=12, i2c_id=0, freq=100000, addr=ADDR, backlight=True):
...
lcd = HD44780_PCF8574(scl=13, sda=12)
btn = Pin(16, Pin.IN)
...
async def app_loop():
    ...
    while True:
        ...
        if v != last and utime.ticks_diff(now, last_change) >= debounce_ms:
            last = v
            last_change = now
            dev.publish("ds/btn", "1" if v else "0")

        await asyncio.sleep_ms(10)

async def main():
    await wifi_connect("SSID","PW")
